...
if [ "$(id -u)" -eq 0 ]; then
    printf "ERROR: Security Error: This script should not be run as root\n" >&2
    printf "ERROR: Running Homebrew operations as root can create security vulnerabilities\n" >&2
    printf "ERROR: Please run as a regular user instead\n" >&2
    exit 1
fi
