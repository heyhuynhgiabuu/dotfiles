/**
 * OpenCode Notification Plugin - TypeScript Version
 * Enhanced with proper OpenCode SDK types and improved functionality
 */
...
export const NotificationPlugin: Plugin = async ({
  ...
  client.app
    .log({
      body: {
        ...
        level: "info",
        message: `🔔 Notification Plugin loaded for: ${project?.id || directory}`,
      },
    })
    .catch(() => {}); // Non-blocking with error handling

  const extractSummary = (text: string | null | undefined): string | null => {
    if (!text) return null;
  ...
  const notify = async (summary: string): Promise<void> => {
  ...
  // Event throttling + deduplication with proper types
  let lastMessage: { text: string | null; time: number } = {
    text: null,
    time: 0,
  };
